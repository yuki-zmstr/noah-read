...
class AgentDecision(BaseModel):
    ...
    action: str = Field(description="The action to take: 'book_recommendation', 'schedule_reading', 'create_review', 'conversation', or 'FINAL_ANSWER'")
...
"""
    ...
    def _handle_conversation(self, input_str: str) -> str:
        ...
        response = self.llm.invoke([SystemMessage(content=conversation_prompt)])
    ...
    def run(self, user_input: str, max_iterations: int = 5) -> str:
        ...
        for iteration in range(max_iterations):
            ...
            formatted_prompt = self.react_prompt.format(
                ...
                chat_history=self.chat_history[:-1],  # All except current input
                input=user_input,
            ...
            if decision.action in self.tool_map:
                ...
                # Check if this observation completes the task
                if any(indicator in observation.lower() for indicator in ["✅", "here are", "scheduled", "created"]):
